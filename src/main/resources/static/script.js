function kjop() {
    /* Henter inputtene fre HTML siden */
    ...
    /* Deklarerer et array og definerer verdiene til et objekt */
    let ordre = {
        film : film,
        antall : antall,
        fornavn : fornavn,
    ...
    */
    ...
    if (!telefonnrRegex.test(telefonnr)) {
        feilmld += "Telefonnummer<br>";
    }
    if (!epostRegex.test(epost)) {
        feilmld += "Epost<br>";
    }

    /* Hvis feilmeldingen er tom string, betyr dette at alle input feltene er godkjente og bilettene kjøpes */
    if (feilmld === "") {
        $.post("/tilServer", ordre, function (){
            hent();
        });

        // Reset av input feltene etter vellykket kjøp
        $("#velgfilm").val("");
        $("#antall").val("");
        $("#fornavn").val("");
        $("#etternavn").val("");
        $("#telefonnr").val("");
        $("#epost").val("");
        /* Hvis ikke feilmeldingene er en tom string, betyr dette at det er en feil i en eller flere av input feltene */
    } else {
